...
if (!function_exists('dump')) { 
    function dump($product, $label=''){ echo '<pre>'; print_r($label); print_r($product); echo"</pre>"; } 
}
/////////////////////////
/////////////////////////
...
function scd_custom_product_basic_load() {
    ...
    if ($reg['priceByCurrency']) {
        add_meta_box('scd_render_custom_product_basic_metabox', __('SCM - Set Currency per Product', 'ch_scd_woo'), 'scd_render_custom_product_basic_metabox', 'product', 'normal', 'high');
...
<div class="scd_cpp">
    ...
    <h4 style="margin-right: -15px;"><?php _e('Set currency : ', 'ch_scd_woo'); ?></h4>
    ...
    if(apply_filters('is_scd_multivendor',false)) 
    {
        if ('administrator' != $scd_getUserRole) {
            ...
            ?>
    <div class="help-tip">
        <p> <?php _e('You have (' . $choi . ') choice(s).', 'ch_scd_woo'); ?> </p>
    </div>
    <?php 
        } 
    }
...
<div id="mySetPrice">
    ...
    <p>
        ...
        <select id="scd_regularCurrency" name='scd_regularCurrency' style="margin-right: 7px;width: 148px;"
            data-placeholder=" <?php _e('Regular price', 'ch_scd_woo'); ?> " class="scd_price_select">
        ...
        <input id="scd_regularPriceCurrency" name='scd_regularPriceCurrency'
            placeholder=" <?php _e('Set Regular Price. (e.g. 59.34)', 'ch_scd_woo'); ?> " style="width: 55%;border-radius: 3px;border: 1px solid #ccc;"
    ...
    <p>
        ...
        <select id="scd_saleCurrency" name='scd_saleCurrency' style="margin-right: 7px;width: 148px;"
            data-placeholder=" <?php _e('Sale price', 'ch_scd_woo'); ?> " class="scd_price_select" disabled="disabled">
        ...
        <?php
                ...
                ?>
        <input id="scd_salePriceCurrency" name='scd_salePriceCurrency' placeholder=" <?php _e('Set Sale Price. (e.g. 32.05)', 'ch_scd_woo'); ?> "
...
function scd_convert_price_in_html_markup($price_html, $price_formatted, $args, $unformatted_price) {
    ...
    if(in_array('wcfm-dashboard-page', get_body_class())){   // check if this is the wcfm dashboard page
        ...
        if (wcfm_is_vendor() && ($wp->query_vars["wcfm-orders-details"] === null)) {
            # code...
...
function scd_init_filters() {
   ...
   //       in scenarios where some items in cart have custom prices set using SCM
   ...
   //       by WooCommerce and specify the custom price set using SCM. Woocommerce will then
   //       use this price in all its calculations. SCM only has to convert the result and does
...
/**
 * Get the target currency set by SCM
 */
...
/**
 * Get the SCM conversion rate between two currencies
 * 
			...
			//return;
        {
            ...
            foreach( $order->get_items( array( 'line_item', 'tax', 'shipping', 'fee', 'coupon'  ) ) as $item_id => $item ) 
            {
                ...
                if( $item['type'] === 'line_item' )
                {
                    ...
                    // Check if SCM uses a price per currency setting for this product
        /*            if(scd_is_custom_price_defined_for_currency($product_id, $target_currency))
                    {
                        // Get the product price from the id
                        list($regprice, $saleprice) = scd_get_product_custom_price_for_currency($product_id, $target_currency);

                        // SCM uses a custom price per currency setting for this product. 
...
<form method="POST">
    ...
    <div id="endwid1" class="scd_widget1">
        ...
        <label for="scd_widget_selector" style="display:none;"> <?php _e('Select a country here...', 'ch_scd_woo'); ?> </label>
    </div>
</form>
...
function scd_pay_for_order( $order_id ) {
    ...
    echo '<p>' . __( 'Thank you for placing your order with us.', 'ch_scd_woo' ) . '</p>';
    echo '<p>' . __( 'You will be redirected to iPay Ghana Payment Gateway checkout page so as to complete your payment.', 'ch_scd_woo' ) . '</p>';
...
function scd_generate_ipay_ghana_wc_checkout_form( $order_id ) {
    ...
    $order->add_order_note( __( 'Order placed successfully; customer has been redirected to the iPay Ghana Payment Gateway checkout page.', 'ch_scd_woo' ) );
    $order->update_status( 'on-hold', __( 'Awaiting Mobile Money payment.<br>', 'ch_scd_woo' ) );
    WC()->cart->empty_cart();
    ...
    <button type="submit" class="button alt" id="submit-payload-to-ipay-ghana-wc-payment-gateway-checkout-url">' . __( 'Checkout with iPay Ghana', 'ch_scd_woo' ) . '</button>
    </div>
    </form>';
}
...
/******************************************************************
  Disable SCM Price Field to add Booking product
********************************************************************/
...
/******************************************************************
  End Disable SCM Price Field to add Booking product
********************************************************************/
...
//If you wish to enable PayPal Standard by default on new sites
...
*/
/*
if ( !function_exists('scd_url_currency_param_redirect') ) {
    function scd_url_currency_param_redirect(){
        $currency = scd_get_target_currency();
...
add_action('template_redirect', 'scd_url_currency_param_redirect'); 
*/