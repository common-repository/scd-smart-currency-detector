<?php
/*
*Plugin Name: SCM - Smart Currency Manager
...
function scd_lazone_load_domaine() {
    load_plugin_textdomain( 'ch_scd_woo', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
}
add_action( 'plugins_loaded', 'scd_lazone_load_domaine' );


add_action( 'wp_ajax_current_user_can_manage', 'current_user_can_manage' );
add_action( 'wp_ajax_nopriv_current_user_can_manage', 'current_user_can_manage' ); 
function current_user_can_manage(){
	
	$s_address['current_user_can_manage'] = false;
	
	if ( current_user_can( 'manage_options' ) ) {
		$s_address['current_user_can_manage'] = true;
	}
...
function scd_mobilewidget_enable_config_check() {
    $pages_options = get_option('scd_currency_options');
	
    if(  $pages_options['fallbackPages'] == 0 || $pages_options['fallbackPages']==["All"] ){
		
        return scd_get_bool_option('scd_currency_options', 'mobilewidget');
        
    }
    
        foreach ($pages_options['fallbackPages'] as $pag) {
            if(  $pag == get_the_title()  ){
...
function scd_add_scripts_to_post() {
   ...
   wp_enqueue_script('ch_scd_fetch', trailingslashit(plugins_url("", __FILE__)) . "js/scd_fetchdata.js", array('ch_scd_maps_js','ch_scd_defaultdata_js'), '5.5.3', true);
	...
	if(function_exists('wcfm_is_vendor')){
        ...
        if(scd_mobilewidget_enable_config_check() == "1" && !in_array('wcfm-dashboard-page', get_body_class())){
        ...
        if(scd_mobilewidget_enable_config_check() == "1" && !is_page(wcmp_vendor_dashboard_page_id())){
            wp_enqueue_script('ch_scd_mobile_widget', trailingslashit(plugins_url("", __FILE__)) . "js/scd_mobile_widget.js", array('ch_scd_postready'), '5.5.3', true);
        }
    }elseif(function_exists('wcv_is_vendor_dashboard')){

        if(scd_mobilewidget_enable_config_check() == "1" && !wcv_is_vendor_dashboard()){
            wp_enqueue_script('ch_scd_mobile_widget', trailingslashit(plugins_url("", __FILE__)) . "js/scd_mobile_widget.js", array('ch_scd_postready'), '5.5.3', true);
        }
    }elseif(function_exists('dokan_is_seller_dashboard')){

        if(scd_mobilewidget_enable_config_check() == "1" && !in_array('dokan-dashboard', get_body_class())){
            wp_enqueue_script('ch_scd_mobile_widget', trailingslashit(plugins_url("", __FILE__)) . "js/scd_mobile_widget.js", array('ch_scd_postready'), '5.5.3', true);
        }
    }else{
		if(scd_mobilewidget_enable_config_check() == "1"){
        wp_enqueue_script('ch_scd_mobile_widget', trailingslashit(plugins_url("", __FILE__)) . "js/scd_mobile_widget.js", array('ch_scd_postready'), '5.5.3', true);
        }
	}
...
$filter=false;    
    ...
    wp_localize_script('ch_scd_postready', 'settings', array(
        ...
        'currencyVal' => '',
         ...
         'fallbackPositionV' => $currency_options['fallbackPositionV'],
         'fallbackPositionH' => $currency_options['fallbackPositionH'],
	...
	wp_localize_script( 'ch_scd_mobile_widget', 'scd_get_php_vars', [
...
function scd_settings_tabs() {
   $tabs=array(
       'settings'=>array('id'=>'settings','label'=> __( 'GENERAL SETTINGS', 'ch_scd_woo' ),'class'=>'nav-tab','page'=>'scd_options_page','name'=>'scd_general_options','submit'=>true),
       'currencies'=>array('id'=>'currencies','label'=> __( 'CURRENCIES SETTINGS', 'ch_scd_woo' ),'class'=>'nav-tab','page'=>'scd_options_page','name'=>'scd_currency_options','submit'=>true),
       'help'=>array('id'=>'help','label'=> __( 'HELP & SUPPORT', 'ch_scd_woo' ),'class'=>'nav-tab','page'=>'scd_options_page','name'=>'scd_help_options','submit'=>false)
   ); 
...
function scd_render_scd_display() {
    ...
    if ($active_tab == $key) {
        ...
        <form method="post" action="options.php" id="<?php echo $tab_item['name']; ?>-form">
            ...
            //}  else {
             submit_button( __('Save Changes', 'ch_scd_woo' ), 'button-primary scd_save');    
            //}
          }
            echo("<hr />");
            ?>
...
function scd_plugin_menu() {
    ...
    add_submenu_page(
            'woocommerce', 'SCM Settings', __('SCM Currencies', 'ch_scd_woo' ), 'manage_options', 'scd_options_page', 'scd_render_scd_display'
    );
}
...
function scd_unactivate_notice() { 
    ...
    <div class="notice notice-warning is-dismissible">
        <p style="color: brown; font-size: 1.3em;"><strong> <?php echo __( 'SCM Smart-Currency-Manager', 'ch_scd_woo' ) ?> </strong></p>
        <?php $notice=apply_filters('scd_notice', __('You are using the free version of SCM-Smart-Currency-Manager, please get the Premium version', 'ch_scd_woo') . '
            <a href= "https://gajelabs.com/">' . __('here', 'ch_scd_woo' ) . '</a>' . __( 'to access to more features', 'ch_scd_woo' ));
        echo $notice;
        ?>
    </div>
    <?php 
...
// create the links
	if ( $file == $plugin ) {
		...
		$scdlinks=apply_filters('scd-links',array(
			'<a href="' . $supportlink . '" target="__blank"> <span class="scd-support scd-dashb-item" title="' . __( 'SCM Support', 'ch_scd_woo' ) . '"></span>' . __( 'Support', 'ch_scd_woo' ) . '</a>',
			'<a href="' . $standardlink . '" target="__blank"><span class="scd-standard scd-dashb-item" title="Get SCM Standard">Standard</span></a>',
			'<a href="' . $marketplacelink . '" target="__blank"><span class="scd-marketplace scd-dashb-item" title="' . __( 'Get SCM Marketplace variant', 'ch_scd_woo' ) . '">' . __( 'Marketplace variant', 'ch_scd_woo' ) . '</span></a>',
			'<a href="' . $marketplaceprolink . '" target="__blank"><span class="scd-marketplacepro scd-dashb-item" title="' . __( 'Get SCM Marketplace pro', 'ch_scd_woo' ) . '">' . __( 'Marketplace pro', 'ch_scd_woo' ) . '</span></a>',
			'<a href="' . $tutoriallink . '" target="__blank"><span class="scd-tutorial scd-dashb-item" title="' . __( 'SCM Tutoriel', 'ch_scd_woo' ) . '">' . __( 'Tutorial', 'ch_scd_woo' ) . '</span></a>'
		));
...
function sample_admin_notice_warning() {
    ...
    <div class="notice notice-warning" >
        <p><?php echo '<strong style="color: brown; font-size: 1.3em;" > SCM Smart-Currency-Manager </strong> : if you are using other SCM variant, please make sure that it is at least to version 4.7.8.'; ?></p>
    </div>
    <?php
}
...
return '<form method="POST">
			...
			<div id="endwid">
				...
				<label for="scd_widget_selector" style="display:none;">' . __('Select a country here...', 'ch_scd_woo') . '</label>
			</div>
...
function my_plugin_action_links( $actions ) {
   $actions[] = '<a href="'. esc_url( get_admin_url(null, 'admin.php?page=scd_options_page') ) .'">' . __( 'Settings', 'ch_scd_woo' ) . '</a>';
